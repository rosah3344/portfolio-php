...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["add_resource"])) {
    ...
    $query = "INSERT INTO resources (user_id, resource_name, description, category) VALUES ($user_id, '$resource_name', '$description', '$category')";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["edit_resource"])) {
    ...
    $description = mysqli_real_escape_string($conn, $_POST["description"]);
    $category = mysqli_real_escape_string($conn, $_POST["category"]);
    $query = "UPDATE resources SET resource_name='$resource_name', description='$description', category='$category' WHERE id=$resource_id AND user_id=$user_id";
    mysqli_query($conn, $query);
    header("Location: resources.php");
    exit();
}
...
if (isset($_GET["delete"])) {
    ...
    $query = "DELETE FROM resources WHERE id=$resource_id AND user_id=$user_id";
    mysqli_query($conn, $query);
    header("Location: resources.php");
    exit();
}
...
$resources_query = mysqli_query($conn, "SELECT * FROM resources WHERE user_id=$user_id");
?>
...
<head>
    ...
    <title>Cultive8 - Resources</title>
    <!-- External Libraries -->
...
<body>
        ...
        <main style="margin-left: 250px; width: calc(100% - 250px); padding: 20px;">
            ...
            <header style="display: flex; justify-content: space-between; align-items: center; padding: 15px 30px; background: rgba(255, 255, 255, 0.9); border-radius: 10px; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.1), 0 0 15px rgba(76, 175, 80, 0.5); margin-bottom: 20px; backdrop-filter: blur(5px); animation: neonGlow 2s infinite alternate; transition: background 0.5s ease;">
                <div style="display: flex; align-items: center; gap: 15px;">
                    ...
                    <h1 style="font-family: 'Georgia', serif; color: #2e7d32; font-size: 1.8em; margin: 0; text-shadow: 0 2px 4px rgba(0, 0, 0, 0.2);">Resources</h1>
                </div>
            ...
            <!-- Add Resource Form -->
            <section style="margin-bottom: 30px; background: rgba(255, 255, 255, 0.9); padding: 20px; border-radius: 15px; box-shadow: 0 5px 15px rgba(0, 0, 0, 0.1);">
                <h2 style="color: #2e7d32; font-size: 1.8em; margin-bottom: 15px; font-family: 'Georgia', serif; text-shadow: 0 2px 4px rgba(0, 0, 0, 0.2);">Add New Resource</h2>
                <form method="POST" style="display: flex; gap: 10px; flex-wrap: wrap;">
                    <input type="text" name="resource_name" placeholder="Resource Name" required style="padding: 10px; border: 1px solid #ccc; border-radius: 5px; flex: 1; min-width: 200px;">
                    <input type="text" name="description" placeholder="Description" required style="padding: 10px; border: 1px solid #ccc; border-radius: 5px; flex: 1; min-width: 200px;">
                    <input type="text" name="category" placeholder="Category" required style="padding: 10px; border: 1px solid #ccc; border-radius: 5px; flex: 1; min-width: 200px;">
                    <button type="submit" name="add_resource" style="padding: 10px 20px; background: #4CAF50; color: #fff; border: none; border-radius: 5px; cursor: pointer; box-shadow: 0 0 10px rgba(76, 175, 80, 0.5); animation: pulse 2s infinite;">Add Resource</button>
                </form>
            </section>

            <!-- Resources Table -->
            <section style="background: rgba(255, 255, 255, 0.9); padding: 20px; border-radius: 15px; box-shadow: 0 5px 15px rgba(0, 0, 0, 0.1);">
                <h2 style="color: #2e7d32; font-size: 1.8em; margin-bottom: 15px; font-family: 'Georgia', serif; text-shadow: 0 2px 4px rgba(0, 0, 0, 0.2);">Your Resources</h2>
                <table style="width: 100%; border-collapse: collapse;">
                    <thead>
                        <tr style="background: #4CAF50; color: #fff;">
                            <th style="padding: 10px; border-bottom: 2px solid #388e3c;">Resource Name</th>
                            <th style="padding: 10px; border-bottom: 2px solid #388e3c;">Description</th>
                            <th style="padding: 10px; border-bottom: 2px solid #388e3c;">Category</th>
                            <th style="padding: 10px; border-bottom: 2px solid #388e3c;">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php while ($resource = mysqli_fetch_assoc($resources_query)) { ?>
                            <tr class="resource-row" data-id="<?php echo $resource['id']; ?>">
                                <td style="padding: 10px; border-bottom: 1px solid #ddd;" contenteditable="false" class="editable" data-field="resource_name"><?php echo htmlspecialchars($resource['resource_name']); ?></td>
                                <td style="padding: 10px; border-bottom: 1px solid #ddd;" contenteditable="false" class="editable" data-field="description"><?php echo htmlspecialchars($resource['description']); ?></td>
                                <td style="padding: 10px; border-bottom: 1px solid #ddd;" contenteditable="false" class="editable" data-field="category"><?php echo htmlspecialchars($resource['category']); ?></td>
                                <td style="padding: 10px; border-bottom: 1px solid #ddd;">
                                    <button onclick="editRow(this)" style="padding: 5px 10px; background: #4CAF50; color: #fff; border: none; border-radius: 5px; cursor: pointer; margin-right: 5px;">Edit</button>
                                    <button onclick="saveRow(this)" style="padding: 5px 10px; background: #388e3c; color: #fff; border: none; border-radius: 5px; cursor: pointer; margin-right: 5px; display: none;">Save</button>
                                    <a href="resources.php?delete=<?php echo $resource['id']; ?>" style="padding: 5px 10px; background: #d32f2f; color: #fff; border: none; border-radius: 5px; text-decoration: none;">Delete</a>
                                </td>
                            </tr>
                        <?php } ?>
                    </tbody>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .editable[contenteditable="true"] { background: #e8f5e9; }
        ...
        @media (max-width: 768px) {
            ...
            form input, form button { width: 100%; margin-bottom: 10px; }
        }
    </style>
    ...
    <script>
        ...
        function editRow(button) {
            ...
            row.querySelectorAll('.editable').forEach(cell => cell.setAttribute('contenteditable', 'true'));
        ...
        function saveRow(button) {
            ...
            const resourceName = row.querySelector('[data-field="resource_name"]').textContent;
